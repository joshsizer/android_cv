...
/*
 * Most of this code is courtesy of Team 254. Thank you for doing the heavy lifting.
 */
enum DisplayMode {
  DISP_MODE_RAW = 0,
  DISP_MODE_THRESH = 1,
...
static void ensureJniRegistered(JNIEnv *env) {
  ...
  jclass targetsInfoClass =
      env->FindClass("com/team341/daisycv/vision/ImageProcessor$TargetsInfo");
  sNumTargetsField = env->GetFieldID(targetsInfoClass, "numTargets", "I");
  sTargetsField = env->GetFieldID(
      targetsInfoClass, "targets",
      "[Lcom/team341/daisycv/vision/ImageProcessor$TargetsInfo$Target;");
  jclass targetClass =
      env->FindClass(
          "com/team341/daisycv/vision/ImageProcessor$TargetsInfo$Target");

  sCentroidXField = env->GetFieldID(targetClass, "x", "D");
  sCentroidYField = env->GetFieldID(targetClass, "y", "D");
  sWidthField = env->GetFieldID(targetClass, "width", "D");
  sHeightField = env->GetFieldID(targetClass, "height", "D");
}

extern "C" void process(JNIEnv *env, int tex1, int tex2, int w, int h,
                        int mode, int h_min, int h_max, int s_min,
                        int s_max, int v_min, int v_max,
                        jobject destTargetInfo, bool log_stuff) {
  auto targets = processImpl(w, h, tex2, static_cast<DisplayMode>(mode), h_min,
                             h_max, s_min, s_max, v_min, v_max, log_stuff);
...
Java_com_team341_daisycv_vision_ImageProcessor_processImage(
    JNIEnv *env,
    jclass cls,
    jint tex1,
    jint tex2,
    ...
    jint v_min,
    jint v_max,
    jobject destTargetInfo, jboolean log_stuff) {
  process(env,
          tex1,
          tex2,
          w,
